...
void Graph::removeEdge(std::string v1, std::string v2) {
    ...
    if(indexOfV1Edge < 0 || indexOfV2Edge < 0) return;

    // removing
    this->graph[indexV1].edges.erase( this->graph[indexV1].edges.begin() + indexOfV1Edge);
    this->graph[indexV2].edges.erase( this->graph[indexV2].edges.begin() + indexOfV2Edge);
}
